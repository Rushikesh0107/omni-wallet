...
import { updatePreview, resetPreview } from "@/store/slice/card-preview.slice";
...
const cardFormSchema = z.object({
  ...
  fullName: z
    .string({ message: "Card holder name is required" })
    .min(1, { message: "Card holder name must be at least 1 character" })
    .max(100, { message: "Card holder name must be at most 100 characters" }),
  expiryMonth: z
    .string({ message: "Expiry month is required" })
    .min(1, { message: "Expiry month must be at least 1 character" })
    .max(2, { message: "Expiry month must be at most 2 characters" }),
  expiryYear: z
    .string({ message: "Expiry year is required" })
    .min(1, { message: "Expiry year must be at least 1 character" })
    .max(4, { message: "Expiry year must be at most 4 characters" }),
  cvv: z
    .string({ message: "CVV is required" })
    .min(1, { message: "CVV must be at least 1 character" })
    .max(3, { message: "CVV must be at most 3 characters" }),
});
...
const CardForm = ({ type }: { type: string }) => {
  ...
  const preview = useAppSelector((state) => state.cardPreview);

  const watchedValues = form.watch();

  useEffect(() => {
    dispatch(updatePreview(watchedValues));
                ...
                render={({ field }) => (
                  <Field>
                    ...
                    <Input
                      ...
                      id="expiry-month"
                      aria-invalid={
                        form.formState.errors.expiryMonth ? true : false
                      }
                      onChange={(e) => {
                        if (
                    ...
                    {form.formState.errors.expiryMonth && (
                      <FieldError
                        errors={[form.formState.errors.expiryMonth]}
                      />
                    )}
                  </Field>
                )}
                ...
                render={({ field }) => (
                  <Field>
                    ...
                    <Input
                      ...
                      id="expiry-year"
                      aria-invalid={
                        form.formState.errors.expiryYear ? true : false
                      }
                      onChange={(e) => {
